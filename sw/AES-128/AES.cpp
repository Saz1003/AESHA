...
unsigned int NhanE(unsigned int w)
{
    ...
    unsigned int result = (w << 3) ^ (w << 2) ^ (w << 1);
    // 1110 1001   => 11 1010 0100
    // 1 0000 0000 => 100 0000 000
    if (result >= (256 << 2))
    {
        result = result ^ (0x11b << 2);
    }
...
unsigned int *GiaimaAES(unsigned int s[4], unsigned int Key[4])
{
    unsigned int *w = KeyExpansion(Key);
    unsigned int *state = AddRoundKey(s, &w[40]);
    for (int i = 1; i <= 9; i++)
    {
...
int main()
{
    ...
    // KEY = 1ef2ed3dbb825a776457edc8e12ccec2
    // DATA = 061d7f0c4b212f96a8c7fc51ebfec0d7
    unsigned int w0 = 0x1ef2ed3d, w1 = 0xbb825a77, w2 = 0x6457edc8, w3 = 0xe12ccec2;
